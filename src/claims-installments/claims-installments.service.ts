...
import { ClaimBaseQueryParams, ClaimInstallmentQueryParams } from '../share/dto/claim-query-params.dto';
import { resolveYears, yearCondition, getInstallmentNo, resolveInstallments } from '../share/helpers/date-filter.helper';
import { BaseQueryParams } from '../share/dto/base-query-params.dto';

@Injectable()
export class ClaimsInstallmentsService {
  constructor(
  ...
  async getPolicyYears(q: ClaimInstallmentQueryParams): Promise<number[]> {
  ...
  async getByProduct(q: ClaimInstallmentQueryParams) {
    const years = resolveYears(q);
    const policies = q.policies?.split(',').filter(Boolean) ?? [];
    const installments = resolveInstallments(q);
  ...
  async getByStatus(q: ClaimInstallmentQueryParams) {
    const years = resolveYears(q);
    const policies = q.policies?.split(',').filter(Boolean) ?? [];
    const installments = resolveInstallments(q);
    const policyEffectiveYears = this.resolvePolicyEffectiveYears(q);
  ...
  async getCards(q: ClaimInstallmentQueryParams) {
    const years = resolveYears(q);
    const policies = q.policies?.split(',').filter(Boolean) ?? [];
    const installments = resolveInstallments(q);
    const policyEffectiveYears = this.resolvePolicyEffectiveYears(q);
  ...
  async getPolicies(q: ClaimInstallmentQueryParams) {
    const years = resolveYears(q);
    const policies = q.policies?.split(',').filter(Boolean) ?? [];
    const installments = resolveInstallments(q);
    const policyEffectiveYears = this.resolvePolicyEffectiveYears(q);
  ...
  async getClaims(q: ClaimInstallmentQueryParams) {
    const years = resolveYears(q);
    const policies = q.policies?.split(',').filter(Boolean) ?? [];
    const installments = resolveInstallments(q);
    const policyEffectiveYears = this.resolvePolicyEffectiveYears(q);
  ...
  async getRelatedPremiums(q: ClaimInstallmentQueryParams) {
    const years = resolveYears(q);
    const policies = q.policies?.split(',').filter(Boolean) ?? [];
    const installments = resolveInstallments(q);
    const policyEffectiveYears = this.resolvePolicyEffectiveYears(q);
    ...
    if (q.search) {
      qb.andWhere(
        '(p."PolicyNumber" ILIKE :s OR pol.subscriber_name ILIKE :s OR p."Receipt" ILIKE :s)',
        { s: `%${q.search}%` },
      );
    }
    ...
    if (pageSize === 1) {
      console.log(`[Related-Premiums Installment Count API] Total: ${total} premium records for ${policyNumbers.length} claim policies`);
    } else {
